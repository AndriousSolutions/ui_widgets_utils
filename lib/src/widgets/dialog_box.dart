  ...
  VoidCallback? onPressed,
    ...
    TextButton(
      ...
      onPressed: () {
        ...
        Navigator.pop<T>(context, optionResult ?? result);
      },
...
class _SimpleDialog with DialogOptions {
  ...
  SimpleDialog show() {
    return SimpleDialog(
      title: Text(title ?? ' '),
      children: [Row(children: _listOptions(context))],
    );
  }
}
...
mixin DialogOptions {
  ...
  List<Widget> _listOptions(BuildContext context) {
    final opList = <Widget>[];
...
Widget simpleOption(BuildContext context, Option option) => SimpleDialogOption(
      key: option.key,
      onPressed: () {
        if (option.onPressed != null) {
          option.onPressed!();
